...
public class OAuthController {
	...
	@Autowired
	public OAuthController(UserServiceImpl userService, CognitoService cognitoService) {
	...
	public ResponseEntity<ValidateEmailResponseDto> confirmEmail(@RequestParam String email, @RequestParam String code) {
		try {
			this.cognitoService.confirmEmail(email, code);
			return ResponseEntity.ok(new ValidateEmailResponseDto(true, email, "Email successfully confirmed."));
		} catch (Exception e) {
	...
	public ResponseEntity<ResendEmailResponseDto> resendEmailVerification(@RequestParam String email) {
		try {
			...
			return ResponseEntity.ok(new ResendEmailResponseDto(true, email, "Email successfully resended."));
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ResendEmailResponseDto(false, email, e.getMessage()));
		}
	}
}
